...
本カリキュラムでは Supabase、Django、Looker Studio を組み合わせて使用します。
それぞれのインフラ管理の負荷と管理画面の難易度を整理します。

### Supabase（BaaS）

```
【インフラ管理：不要】
・DBサーバーの構築・運用 → Supabaseが管理
・PostgreSQLのバージョンアップ → 自動
・バックアップ → 自動（Proプラン以上）
・スケーリング → 自動
・セキュリティパッチ → 自動
・SSL証明書 → 自動

【管理画面：Supabase Dashboard】
・Webブラウザで完結
・Table Editor（GUIでテーブル操作）
・SQL Editor（SQLを直接実行）
・ログ確認、接続情報管理
...
```
【インフラ管理：PaaS利用で最小化】
・開発環境 → GitHub Codespaces（自動構築）
・本番環境 → Render / Railway 等のPaaSを推奨

| PaaSサービス | 難易度 | 特徴 |
|---|---|---|
| Render | 低 | Git連携で自動デプロイ |
| Railway | 低 | シンプル、無料枠あり |
| Fly.io | 中 | エッジ対応 |
...
| 項目 | Supabase | Django | Looker Studio |
|---|---|---|---|
| 分類 | BaaS | フレームワーク | BI SaaS |
| 主な用途 | DB管理 | 業務アプリ | データ可視化 |
| インフラ管理 | 不要 | PaaS利用で最小 | 不要 |
| 初期設定 | 数分 | 数時間 | 数分 |
| 操作難易度 | 低 | 低〜中 | 非常に低 |
| カスタマイズ | 限定的 | 自由 | GUI範囲内 |
| 技術知識 | SQL推奨 | Python必要 | 不要 |
| 非エンジニア向け | ○ | ○ | ◎ |
| 料金 | 無料枠あり | PaaS依存 | 無料 |
...
| 用途 | ツール | 対象者 |
|---|---|---|
| データ入力・編集 | Djangoアプリ | 現場スタッフ |
| データ管理・修正 | Django Admin | 管理者・DX担当 |
| DB直接操作 | Supabase Dashboard | 開発者 |
| 分析・レポート | Looker Studio | 経営者・管理職 |
```
...
├──────────────────┼────────────────┼────────────────┤
│ サーバー購入      │ 100〜300万円    │ 不要            │
│ サーバー室・電気代 │ 月10〜30万円    │ 不要            │
│ 保守担当者        │ 月40〜60万円    │ 不要            │
│ 月額費用          │ 固定費大        │ 数千円〜        │
│ 障害対応          │ 自社責任        │ 各サービス提供元 │
└──────────────────┴────────────────┴────────────────┘

→ 中小企業でもエンタープライズ級のインフラを低コストで利用可能
```
