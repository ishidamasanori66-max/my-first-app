...
本カリキュラムでは PWA（Progressive Web App）を標準としていますが、ハードウェア連携や App Store 配布が必要な場合は、ネイティブアプリ開発を検討します。

### PWA vs ネイティブアプリの機能比較

| 機能 | PWA | Capacitor | React Native | Flutter | ネイティブ |
|---|---|---|---|---|---|
| カメラ（写真撮影） | ○ | ◎ | ◎ | ◎ | ◎ |
| カメラ（バーコード/QR） | △ | ◎ | ◎ | ◎ | ◎ |
| GPS（現在地取得） | ○ | ◎ | ◎ | ◎ | ◎ |
...
| 観点 | PWA | Capacitor | React Native | Flutter |
|---|---|---|---|---|
| 言語 | HTML/CSS/JS | HTML/CSS/JS | JavaScript/TS | Dart |
| 学習コスト | 低 | 低 | 高 | 中 |
| カリキュラムとの親和性 | ◎ | ◎ | △ | △ |
| パフォーマンス | △ | ○ | ○ | ◎ |
| UIの自由度 | △ | ○ | ◎ | ◎ |
| 開発速度 | ◎ | ◎ | ○ | ○ |
| 追加学習時間 | 0時間 | 10〜20時間 | 60〜90時間 | 40〜60時間 |

### 各技術の概要

#### Capacitor（推奨：学習コスト最小）

```
【概要】
・Ionic社が開発するネイティブアプリ化フレームワーク
・既存のWebアプリ（Django + HTMX）をそのままラップ
・プラグインでネイティブ機能（Bluetooth, NFC等）を追加

【メリット】
・Django + HTMX のスキルがそのまま活きる
・React / Flutter の学習不要
・既存のWeb画面をそのままアプリ化可能
・段階的にネイティブ機能を追加できる

【デメリット】
・React Native / Flutter より動作がやや重い
・複雑なUIアニメーションは苦手
・大規模・高負荷アプリには不向き

【向いているケース】
・既存のDjango + HTMXアプリをアプリ化したい
・シンプルなBluetooth/NFC連携が必要
・学習コストを最小限に抑えたい
```

#### React Native（Web版もReact化する場合）

```
【概要】
・Meta（Facebook）が開発
・JavaScript/TypeScript でネイティブアプリを構築
・React の知識がそのまま活きる

【メリット】
・Web版（React）とコードを共有可能
・大規模なエコシステムとコミュニティ
・React エンジニアが多く、採用・外注しやすい
・ネイティブに近いパフォーマンス

【デメリット】
・JavaScript + React の学習が必須（60〜90時間）
・本カリキュラムの HTMX 方針から外れる
・npm / ビルド環境の管理が必要

【向いているケース】
・Web版も React に移行する予定がある
・複雑なUI・インタラクションが必要
・React エンジニアを採用予定
```
...
【概要】
・Google が開発
・Dart言語で iOS / Android / Web / デスクトップを統一開発
・独自レンダリングエンジンで高性能UI
...
```
【シンプルなアプリ（情報表示、予約、注文）】
→ Capacitor（既存スキルで対応可能）

【センサー重視（Bluetooth体重計、NFC入退室）】
→ Capacitor または React Native

【本格的なアプリ（複雑なUI、高パフォーマンス）】
→ Flutter または React Native

【Web版もモダン化したい】
→ React + React Native（コード共有）
...
【推奨技術】
・Capacitor：Bluetooth/NFCプラグインで対応可能
・または React Native / Flutter：より本格的な場合
```
...
【推奨技術】
・Capacitor：多くの機能がプラグインで対応可能
・決済を重視するなら React Native も検討
```
...
顧客向けネイティブアプリが必要
    │
    ├─ Bluetooth/NFC/ヘルスケア連携が必要？
    │     │
    │     ├─ Yes（シンプル）→ Capacitor
    │     └─ Yes（高度）→ React Native / Flutter
    │
    ├─ Web版も React に移行する？
    │     │
    ...
    │     └─ Yes → Flutter
    │
    ├─ 既存の Django + HTMX を活かしたい？
    │     │
    │     └─ Yes → Capacitor（推奨）
    │
    └─ 内製リソースがない？
          │
          └─ Yes → 外注（Django APIのみ内製）
```

### Capacitor 導入の概要（参考）

```python
# Django側：既存のHTMXアプリをそのまま使用

# Capacitor プロジェクトの初期化
...
```
【学習ステップ】
1. Capacitor の基本概念（2〜3時間）
2. プロジェクトセットアップ（2〜3時間）
3. 必要なプラグインの導入（3〜5時間）
4. ビルド・デバッグ・配布（3〜5時間）
→ 合計：10〜16時間
```
...
1. まずは PWA（ゴールド〜プラチナ標準）
   → App Store不要、開発コスト最小

2. ネイティブ機能が必要になったら Capacitor
   → 既存スキルを活かして段階的に対応

3. 本格的なアプリが必要なら Flutter / React Native
   → 追加学習は必要だが、高品質なアプリが可能

4. リソースがなければ外注
   → Django API は内製、アプリは専門業者に委託
```

### プラチナまでの技術スタックとの関係

どの技術を選択しても、プラチナまでに構築した技術スタック（Django + HTMX + Supabase）は維持されます。

```
【既存のスタック（プラチナまで）】
Django + HTMX + Supabase + その他モジュール
    ↓
【ネイティブアプリを追加しても】
    ↓
Django + Supabase は維持（バックエンドはそのまま）
```

| 技術選択 | Django | Supabase | HTMX (Web版) | 変更点 |
|---|---|---|---|---|
| Capacitor | 維持 | 維持 | 維持 | ラップするだけ |
| React Native | 維持（API化） | 維持 | 維持可能 | APIエンドポイント追加 |
| Flutter | 維持（API化） | 維持 | 維持可能 | APIエンドポイント追加 |
| 外注 | 維持（API化） | 維持 | 維持可能 | APIエンドポイント追加 |

```
【Capacitor の場合】
┌─────────────┐
│ Capacitor   │ ← 既存Webアプリをラップ
│ (アプリ殻)   │
├─────────────┤
│ Django+HTMX │ ← そのまま
├─────────────┤
...
└──────┬──────┘    └──────┬──────┘
       ...
       └────────┬─────────┘
         ...
         │ Supabase    │ ← そのまま
         └─────────────┘
変更点：Django REST Framework でAPI追加
```
...
【追加するもの】
・Capacitor：設定とビルド環境
・その他：Django REST Framework のAPIエンドポイント

→ 既存資産を活かしたまま、ネイティブアプリを追加できる
→ 「置き換え」ではなく「拡張」
```

### 外注時のコスト優位性

プラチナまで修了した企業は、ネイティブアプリ開発を外注する場合でも大幅なコスト削減が可能です。
...
【プラチナ修了企業が外注する場合】
・アプリ開発（iOS/Android）のみ
  → 既存のDjango APIに接続するだけ
・データベース：Supabase稼働中
・認証・権限：Django側で実装済み
・ビジネスロジック：Django側で実装済み
・管理画面：Django Admin稼働中
→ 100万〜300万円程度
```
...
・自社でシステムの「核」を持っている
・外注は「UIの殻」だけで済む
・費用は1/5〜1/10に抑えられる可能性
・発注者として対等な立場で交渉できる
・保守・運用も自社でコントロール可能

→ 「全部外注」と「コアは内製、UIだけ外注」では
  コストもリスクも全く違う
```
